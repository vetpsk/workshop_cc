...
lm1 <- lm(numberculled ~ lifeprodnculled + numberproductiondaysculled + 
            fatculled + proteinculled + numberheifers + year, data = df1, model = TRUE)
summary(lm1)
anova(lm1)

lm2 <- glm(numberculled ~ lifeprodnculled + numberproductiondaysculled + 
             fatculled + proteinculled + numberheifers + year, data = df1,family = poisson(link = "log"))
summary(lm2)
anova(lm2)
...
lm3 <- lmer(numberculled ~ lifeprodnculled + numberproductiondaysculled + 
              fatculled + proteinculled + numberheifers + year + (1|HerdIdentifier), data = df1,)
summary(lm3)
anova(lm3)

lm4 <- lmer(numberculled ~ lifeprodnculled + numberproductiondaysculled + 
              fatculled + proteinculled + numberheifers + (1|HerdIdentifier) + (1|year), data = df1,)
summary(lm4)
anova(lm4)

lm5 <- glmer(numberculled ~ lifeprodnculled + numberproductiondaysculled + 
               fatculled + proteinculled + numberheifers + (1|HerdIdentifier) + (1|year), data = df1, family = poisson(link = "log"))
